...
/* #define WATCHDOG */
/*-------------------------------------------------------------------------*
 *  Custom Code:    WATCHDOG - for use with kernel2 or if watchdog is
 *                             running
 *-------------------------------------------------------------------------*
 *  Source Code:    %M%
 *-------------------------------------------------------------------------*
 ...
 *  07/10/99   |  aha  Added system command to call "stop_caps_script"
 *             |       to stop any other custom processes.
 *-------------------------------------------------------------------------*/
...
extern void done();
extern void timeout1();
extern void timeout2();
extern void ignore();
...
failed:
  ...
  {
    len = strlen(line) - 1;                                     /* length less l
inefeed            */
  ...
  /* aha  07-10-99 */
  sprintf(command, "%s/stop_caps_script", getenv("ETC"));
  system(command);

 ...
 *-------------------------------------------------------------------------*/
static char berkley_control_c[] = "%Z% %M% %I% (%G% - %U%)";
...
struct fld_parms fld[] = {
  ...
  {14,45,23,1,&ONE,  "Enter Code",'a'},
  {16,45,23,1,&FOUR, "Enter File Number",'n'},
  {23,20,1,1,&ONE,   "More? (y/n)",'a'}

};
struct fld_parms fld1 = {16,45,23,1, &EIGHT, "Enter Printer", 'a'};
...
main()
{
  ...
  while(1)                                /* begin massive loop              */
  {
    ...
    if(buf[0][0] != 'd')                  /* not display                     */
    {
      ...
      switch(buf[0][0])    
      {
        case 'a':                         /* abort current print job         */
          ...
          sd_cursor(0, 18, 1);
          sd_text(".");
          sprintf(command, "lpc abort %s", op_printer);
          system(command);
          sleep(5);
          break;
        ...
        case 'k':                         /* kill print file                 */
          ...
          sd_cursor(0, 18, 1);
          sd_text(".");
          sprintf(command, "lprm -P%s %s", op_printer, buf[1]);
          system(command);
          sleep(5);
          break;
        ...
        case 's':                         /* stop print file                 */
          ...
          sd_cursor(0, 18, 1);
          sd_text(".");
          sprintf(command, "lpc stop %s", op_printer);
          system(command);
          sleep(5);
          break;
        ...
        case 'r':                         /* restart printing                */
          ...
          sd_cursor(0, 18, 1);
          sd_text(".");
          sprintf(command, "lpc up %s", op_printer);
          system(command);
          sleep(5);
          break;
      }
    ...
    {
      sd_cursor(0,16,4);
      sd_text("Rank  Operator  Job    File Name");
      sd_cursor(0,16,66);
      sd_text("Total Size");
    }
/*
 ...
 */
    ...
    sprintf(command, "lpq -P%s >%s", op_printer, tname);
    system(command);

    t_fd = fopen(tname, "r");
    if (t_fd == 0) continue;              /* open failed                     */
    
    while (fgets(command, 80, t_fd))
    {
      if (memcmp(command, "Rank", 4) == 0) break;
    }
    memset(x, 0, sizeof(x));
    ...
    while(1)
    {
      ...
      ret = sd_more(t, code_to_caps(buf[2][0]));

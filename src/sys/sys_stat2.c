 ...
 *-------------------------------------------------------------------------*/
static char sys_stat2_c[] = "%Z% %M% %I% (%G% - %U%)";
...
register long j;
{
  ...
  if(c->oc_low.oc_last)
  {
    last_queued = oc->oi_tab[c->oc_low.oc_last - 1].oi_on;
  }
  else if(c->oc_med.oc_last)
  {
    last_queued = oc->oi_tab[c->oc_med.oc_last - 1].oi_on;
  }
  else if(c->oc_high.oc_last)
  {
    last_queued = oc->oi_tab[c->oc_high.oc_last - 1].oi_on;
  }
/*
 ...
 */
  if(c->oc_uw.oc_last)
  {
   //last_underway = c->oc_uw.oc_last;
    last_underway = oc->oi_tab[c->oc_uw.oc_last - 1].oi_on;
    if (rf->rf_grp)
    {
      memcpy(last_group, oc->oi_tab[c->oc_uw.oc_last - 1].oi_grp, rf->rf_grp);
    }
  }
/*
 * pickline lock flag
 */
  ...
  {
    sprintf(lines[0][data], " %7.7s", pl[j].pl_name);
  }
  else sprintf(lines[0][data], "  %6d", j + 1);

  sprintf(lines[1][data], "       %c", pickline_inhibit);
  sprintf(lines[2][data], "  %6d",     pl[j].pl_lines_to_go % 1000000);
  sprintf(lines[3][data], "  %6d",     pl[j].pl_units_to_go % 1000000);
  sprintf(lines[4][data], "  %6d",     queue_count);
  sprintf(lines[5][data], "  %6d",     hold_count);
  sprintf(lines[6][data], "  %6d",     active_count);
  sprintf(lines[7][data], "  %6d",     comp_count);
  if (last_underway) 
  {
    sprintf(lines[8][data], "%8.*d", rf->rf_on, last_underway);
  }
  else memset(lines[8][data], 0x20, 8);

  sprintf(lines[9][data], "%8.8s", last_group);
  if (last_queued)
  {
    sprintf(lines[10][data], "%8.*d", rf->rf_on, last_queued);
  }
  else memset(lines[10][data], 0x20, 8);
  sprintf(lines[11][data], "       %c", lock_flag);
  sprintf(lines[12][data], "  %6d",   pend_count);

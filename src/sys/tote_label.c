 ...
 *  Source Date:    %H% - %T%
 *
 *  Description:    Tote Label printer daemon.
 *
 *-------------------------------------------------------------------------*
 *                            Revision history                             |
 *-------------+-----------------------------------------------------------*
 ...
 *  07/22/95   |  shf  Add printer to environment from Gertrude Hawks.
 *  04/19/96   |  tjt  Add printed count.
 *-------------------------------------------------------------------------*/
static char tote_label_c[] = "%Z% %M% %I% (%G% - %U%)";

/*-------------------------------------------------------------------------*
 *  Prints Tote Labels
 *
 *  To Execute:            tote_label <printer>  &
 *
 *  To Test Report Layout: 
 *
 *             tote_label -d [-f=filename] [-copies=n] [-picks=m] [-value=x]
 *                              
 *                             -d indicates report debug mode
 *                             -f=filename for test data file
...
/*------------------------------------------------------------------------*
 *  Specialized Header Files For Tote Labels
 *------------------------------------------------------------------------*/

#include "custom/labels/remarks_text.h"
#include "custom/labels/tote_defines.h"
...
char printer_env[64];                     /* system printer environment      */
...
/*-------------------------------------------------------------------------*
 *  Specialized Header File For Tote Labels
 *-------------------------------------------------------------------------*/
 
#include "custom/labels/tote_label.h"
 ...
 *-------------------------------------------------------------------------*/
  ...
  while (1)
  {
    ...
    sp->sp_sl_order = of_rec->of_on;   /* current order                   */
...
#ifdef DEBUG
  fprintf(stderr, "aplace(%x, %d, %d, %d) [%*.*s]\n",
    field, len, row, col, len, len, field);
#endif

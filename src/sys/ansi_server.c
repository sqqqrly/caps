 ...
 *  03/12/96   |  tjt  Catch signals for exit.
 *  04/25/97   |  tjt  Allow 0x80 - 0xaf (128-175) input bytes.
 *-------------------------------------------------------------------------*/
...
void catcher(int signum);                 /* signal catcher                  */
extern char *getenv();
...
char **argv;
{
  ...
  putenv("_=ansi_server");                /* name of this program            */
  chdir(getenv("HOME"));
...
#ifdef APU
  change_keytab(2, "2  APU  ", 8);        /* override HELP key               */
/*
  memcpy(&keytabs[1][5], "  APU", 5);
*/
#endif
...
register long flag;                       /* flag = 0 is clear screen        */
{
  ...
  if (flag && parent > 0)
  {
    ...
    sleep(1);                             /* alllow graceful termination     */
    kill(parent, SIGKILL);                /* kill parent                     */
  }
  exit(0);
}
/*-------------------------------------------------------------------------*
...
/*
      system("cd $HOME; /usr/hosts/scohanes");
*/
      on_apu = 0;
...
register long len;
{
  ...
  sprintf(text, "%c[0;36;44m", 0x1b);     /* bright blue                     */
  Fputs(text, stdout);
  
  position(23, 0);                        /* last line of screen             */
  screen_out(p, len);                     /* output line to screen           */

  sprintf(text, "%c[0;37;44m", 0x1b);     /* normal color                    */
  Fputs(text, stdout);

  if (focus) focus_on();                  /* replace cursor if input         */
  sprintf(text, "%c[0;37;44m", 0x1b);     /* normal                          */
  Fputs(text, stdout);
  fflush(stdout);

 ...
 *  Source Date:    %H% - %T%
 *
 *  Description:     System status display with remaining picks.
 ...
 *  04/19/96   |  tjt  New short notice names.
 *  08/23/96   |  tjt  Add begin and commit work.
 *  01/11/97   |  tjt  Add purge date and time.
 *  04/18/97   |  tjt  Add language.h and code xlate.
 *-------------------------------------------------------------------------*/
static char sys_stat4_c[] = "%Z% %M% %I% (%G% - %U%)";
...
struct fld_parms fld[] = {
  ...
  {23,29,2,1,&LPL,"Enter Pickline",'a'},
  {23,29,2,1,&ONE,"More? (y/n)",'a'},
  {23,29,2,1,&ONE,"Display Pending? (y/n)",'a'}
};
unsigned char t;
long pickline = -1;
...
register long n;
{
  ...
  memset(temp, 0, 1920);
  memcpy(temp, sys_stat2, sizeof(sys_stat2));
  fix(temp);
  ...
  sprintf(buffer, "%d", uw_count);               /* active orders count      */
  memcpy(&temp[5][19], buffer, strlen(buffer));

  sprintf(buffer, "%d", comp_count);             /* completed orders count   */
  memcpy(&temp[6][19], buffer, strlen(buffer));

  sprintf(buffer, "%d", sp->sp_sh_printed);      /* shorts printed count     */
  memcpy(&temp[7][19], buffer, strlen(buffer));

  temp[5][46] =  sp->sp_oi_mode;          /* order_input status              */

  sprintf(buffer, "%5d", sp->sp_to_count - sp->sp_to_xmit); /* transactions  */
  memcpy(&temp[6][42], buffer, strlen(buffer));

  sprintf(buffer, "%5d", sp->sp_sh_count); /* short queued for printing      */
  memcpy(&temp[7][42], buffer, strlen(buffer));

  temp[5][76] = sp->sp_to_mode;           /* transaction output status       */
  temp[6][76] = sp->sp_to_flag;           /* transaction accum flag          */
  temp[7][76] = sp->sp_sp_flag;           /* shorts printerd flag            */

  memcpy(&temp[8][19], ctime(&oc->of_last_purge), 24);
...
store_data()
{
  ...
  memset(lines, 0x20, LENGTH * PicklineMax * WIDTH);
...
register long j;
{
  ...
  register long size, queue_count, active_count, hold_count;
  ...
  size = (rf->rf_con < ConLength) ? rf->rf_con : ConLength;
  ...
  if (rf->rf_grp) memset(last_group, 0x20, rf->rf_grp);
 ...
 */
  ...
  else sprintf(lines[0][data], "%14d", j + 1);

  sprintf(lines[1][data], "%13c%c", 0x20, pickline_inhibit);
  sprintf(lines[2][data], "%14d",   pl[j].pl_lines_to_go % 1000000);
  sprintf(lines[3][data], "%14d",   pl[j].pl_units_to_go % 1000000);
  sprintf(lines[4][data], "%14d",   queue_count);
  sprintf(lines[5][data], "%14d",   hold_count);
  sprintf(lines[6][data], "%14d",   active_count);
  sprintf(lines[7][data], "%14d",   comp_count);
  if (last_underway) 
  {
    memcpy(&lines[8][data][14 - size], 
      oc->oi_tab[last_underway - 1].oi_con, size);
  }
  if (rf->rf_grp)
  {
    memcpy(&lines[9][data][14 - rf->rf_grp], last_group, rf->rf_grp);
  }
  if (last_queued) 
  {
    memcpy(&lines[10][data][14 - size], 
      oc->oi_tab[last_queued - 1].oi_con, size);
  }
  sprintf(lines[11][data], "%13c%c", 0x20, lock_flag);
  sprintf(lines[12][data], "%14d",   pend_count);
...
display()
{
  ...
  show_top(720);                          /* renew top of screen             */

 ...
 *  Source Date:    %H% - %T%
 *
 *  Description:     System status display. Kmart Version.
 *
 *-------------------------------------------------------------------------*
 *                            Revision history                             |
 *-------------+-----------------------------------------------------------*
 ...
 *  06/28/95   |  tjt  Add show disable port/pickline.
 ...
 *  04/18/97   |  tjt  Add language and code xlate.
 *-------------------------------------------------------------------------*/
static char sys_stat3_c[] = "%Z% %M% %I% (%G% - %U%)";
...
register long n;
{
  ...
  memset(temp, 0, 1920);
  memcpy(temp, sys_stat3, sizeof(sys_stat3));
  fix(temp);
  ...
  memcpy(&temp[5][23], buffer, strlen(buffer));

  sprintf(buffer, "%6d", comp_count);            /* completed orders count   */
  memcpy(&temp[6][23], buffer, strlen(buffer));

  temp[7][28] = sp->sp_running_status;    /* system configured               */


  temp[5][72] = sp->sp_sp_flag;           /* shorts printed flag             */

  sprintf(buffer, "%5d", sp->sp_sh_count);  /* short queued for printing     */
  memcpy(&temp[6][68], buffer, strlen(buffer));

  sprintf(buffer, "%5d", sp->sp_sh_printed); /* shorts printed count         */
  memcpy(&temp[7][68], buffer, strlen(buffer));
...
register long j;
{
  ...
  register long queue_count, active_count, hold_count, last_group_count;
  ...
  last_group_count = queue_count = active_count = hold_count = 0;
 ...
 */
  if(c->oc_uw.oc_last)
  {
    ...
    if (rf->rf_grp)
    {
      ...
      for (m = 0, o = oc->oi_tab; m < oc->of_size; m++, o++)
      {
        if (o->oi_queue < OC_HIGH || o->oi_queue > OC_LOW) continue;
        if (o->oi_pl != j+1) continue;
        if (memcmp(o->oi_grp, last_group, rf->rf_grp) != 0) continue;
        last_group_count++;
      }
    }
  }
 ...
 */
  ...
  if (flag)
  {
    ...
    lock_flag = caps_to_code(lock_flag);
  ...
  sprintf(lines[2][data], "  %6d",     pl[j].pl_units_to_go % 1000000);
  sprintf(lines[3][data], "  %6d",     queue_count);
  sprintf(lines[4][data], "  %6d",     active_count);
  sprintf(lines[5][data], "  %6d",     comp_count);
  if (last_underway) sprintf(lines[6][data], " %7.*d", 
    rf->rf_on, last_underway);
  else memset(lines[6][data], 0x20, 8);
  sprintf(lines[7][data], "%8.8s", last_group);
  sprintf(lines[8][data], "  %6d", last_group_count);
